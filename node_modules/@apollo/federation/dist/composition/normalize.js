    ...
    const schemaWithDefaultRootTypes = graphql_1.visit(schemaWithoutConflictingDefaultDefinitions, {
        ...
        ObjectTypeDefinition(node) {
            if (node.name.value in rootOperationTypeMap ||
                ...
                return Object.assign({}, node, { name: Object.assign({}, node.name, { value: rootOperationTypeMap[node.name.value] || node.name.value }), kind: graphql_1.Kind.OBJECT_TYPE_EXTENSION });
            }
            return;
        },
        ObjectTypeExtension(node) {
            if (node.name.value in rootOperationTypeMap ||
                defaultRootOperationNames.includes(node.name.value)) {
                return Object.assign({}, node, { name: Object.assign({}, node.name, { value: rootOperationTypeMap[node.name.value] || node.name.value }) });
            }
            return;
        },
        NamedType(node) {
            if (node.name.value in rootOperationTypeMap) {
                return Object.assign({}, node, { name: Object.assign({}, node.name, { value: rootOperationTypeMap[node.name.value] }) });
            }
            return;
        },
    });
...
function replaceExtendedDefinitionsWithExtensions(typeDefs) {
    const typeDefsWithExtendedTypesReplaced = graphql_1.visit(typeDefs, {
        ObjectTypeDefinition(node) {
            const isExtensionDefinition = utils_1.findDirectivesOnTypeOrField(node, 'extends')
                .length > 0;
            ...
            return Object.assign({}, node, (filteredDirectives && { directives: filteredDirectives }), { kind: graphql_1.Kind.OBJECT_TYPE_EXTENSION });
        },
    });
